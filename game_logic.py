...
def check_events(column,board,pause):
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            pygame.quit()
            quit()
        if event.type == pygame.KEYDOWN:
            check_keydown_events(event,column,board,pause)
        if event.type == pygame.KEYUP:
            check_keyup_events(event,column)

# Checks to see if any buttons were pressed.
def check_keydown_events(event,column,board,pause):
...
def add_to_board(settings, screen, column,board,score):
    for block in column:
    ...
    for block in column:
        blocks.append(block)
    columns = check_matches(settings, screen, blocks, board, score)
    blocks = move_down(settings, screen, columns, board, score)
    while len(blocks) != 0:
        columns = check_matches(settings, screen, blocks, board, score)
        blocks = move_down(settings, screen, columns, board, score)
    column.empty()
...
def move_down(settings, screen, columns, board, score):
    blocks = set()
    for x in columns:
        y = 10
        while y != -1:
...
def update_column(settings,screen,column,board,score):
    ...
    if pygame.time.get_ticks() - column.sprites()[0].last >= column.sprites()[0].tick_length:
        if lowest.check_bottom(board):
            ...
            add_to_board(settings, screen, column, board, score)
...
def draw_grid(screen,settings):
    ...
    for x in range(0, 13):
        pygame.draw.line(screen, settings.grid_color, (leftx, topy+x*settings.block_width),
                         (rightx, topy+x*settings.block_width), 5)
    for x in range(0, 7):
        pygame.draw.line(screen, settings.grid_color, (leftx+x*settings.block_width, topy),
                                  (leftx+x*settings.block_width, bottomy), 5)

# Displays "YOU LOSE" text at the end of the game.
def lose_screen(settings,screen):
    font = pygame.font.Font(None, 100)
    text = font.render('YOU LOSE', True, settings.lose_color)
    screen.blit(text, (settings.screen_width/2-text.get_rect().centerx, settings.screen_height/2-text.get_rect().centery))
    pygame.display.flip()

# Displays "PAUSED" when the game is paused.
def pause_screen(settings,screen):
    font = pygame.font.Font(None, 100)
    text = font.render('PAUSED', True, settings.lose_color)
    screen.blit(text, (settings.screen_width/2-text.get_rect().centerx, settings.screen_height/2-text.get_rect().centery))
    pygame.display.flip()
